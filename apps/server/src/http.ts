...
const REMOTE_HEALTH_ROUTE = "/api/remote/health";
...
export const remoteHealthRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    const config = yield* ServerConfig;
    if (config.authToken && url.value.searchParams.get("token") !== config.authToken) {
      return HttpServerResponse.text('{"ok":false}', {
        status: 401,
